...
// Generic Color Fills for alphabet mode
const GENERIC_FILLS = [
  "#dd7127CC",
  "#6bbe31CC",
  "#5c6ee1CC",
...
let COMPUTED_FILLS = (function () {
  let i = 0;
  let output = {};
  for (var atom of atoms.split("")) {
    output[atom] = GENERIC_FILLS[i];
    i++;
  }
  return output;
})();

// In alphabet mode, the prefered amount of letters on a line
const ALPHABET_LINE_LENGTH = 10;
...
function ScreenToGrid(x, y) {
  return {
    x: Math.round((x - CANVASWIDTH / 2) / CELLSIZE),
...
function ClosestPoint(path, sx, sy, mindist, allow_control) {
  ...
  let dsquared = (CANVASWIDTH + CANVASHEIGHT) * (CANVASWIDTH + CANVASHEIGHT);
  let best = null;
...
function DrawPath(p, fill, stroke) {
  ...
  for (let i = 0; i < p.length; i++) {
    ...
    if (
      (prev.e == undefined || prev.f == undefined) &&
      ...
      if (!(next.c != undefined && next.d != undefined)) throw "impossible";
      const { x: c, y: d } = GridToScreen(next.c, next.d);
...
function Bisect(p1, p2) {
  ...
  if (p1.e != undefined && p1.f != undefined) {
    ...
    if (p2.c != undefined && p2.d != undefined) {
      ...
      const x =
        0.125 * p1x + 3 * 0.25 * 0.5 * c + 3 * 0.5 * 0.25 * e + 0.125 * p2x;
      const y =
        0.125 * p1y + 3 * 0.25 * 0.5 * d + 3 * 0.5 * 0.25 * f + 0.125 * p2y;
      return { x, y };
    } else {
  ...
  // Linear.
  return { x: Math.round((p1x + p2x) / 2), y: Math.round((p1y + p2y) / 2) };
}
...
function DrawControlPoints(p, highlight) {
  ...
  for (let i = 0; i < p.length; i++) {
    ...
    const DrawCircle = (x, y, width, lwidth, fill, stroke) => {
      ...
      ctx.fillStyle = fill;
      ctx.strokeStyle = stroke;
      ctx.ellipse(x, y, width, width, width, width, 360);
      ctx.fill();
    ...
    if (pt.c != undefined && pt.d != undefined) {
      ...
      DrawCircle(
        c,
        d,
        highlighted && highlight.cd ? CELLSIZE * 0.35 : CELLSIZE * 0.25,
        1,
...
function DrawModeLetter() {
  ...
  ctx.font = "bold 40pt Helvetica,sans-serif";
  ctx.fillStyle = "#cdc";
  ctx.fillText("Letter " + current_letter + "/" + letter_piece, 20, 50);
...
function DrawLetterEx(letter, dx, dy, t) {
  ...
  for (let i = 0; i < letter.p.length; i++) {
    const tt = (t + i * 40) / 20;
    let piece = letter.p[i];
    let fill = COMPUTED_FILLS[piece.a] || "rgba(0,0,0,0.9)";
...
function DrawModeAlphabet() {
  ...
  const YSTART = 18;
  if (grid) DrawGrid(null, null);
  ALPHABET_T++;
  const alphabet = Object.keys(letters);
  let lines = [""];
  let addingtoline = 1;
  for (let letternum = 0; letternum < alphabet.length; letternum++) {
    if (letternum < ALPHABET_LINE_LENGTH * addingtoline && letternum >= (ALPHABET_LINE_LENGTH * addingtoline) - ALPHABET_LINE_LENGTH ) {
      lines[addingtoline - 1] += alphabet[letternum]
    } else {
      // change line number
      addingtoline++
      lines[addingtoline - 1] = alphabet[letternum]
...
function GetSynchronous(url) {
  // XXX queue?
  if (locked) throw "synchronous while locked";
...
function KeyLetter(e) {
  ...
  if (lc in letters) {
    if (e.altKey) {
      ...
      // Testing to see if language matters i.e. press german umlaut
      console.log(lc);
      current_letter = lc;
      letter_piece = 0;
      letter_kern = null;
  ...
  if (ak) {
    ...
    if (e.shiftKey && letter_kern) {
      if (!letters[current_letter].k) letters[current_letter].k = {};
  ...
  switch (e.key) {
    ...
    case ",":
      letter_piece--;
      if (letter_piece < 0) letter_piece = letters[current_letter].p.length - 1;
      break;
    case ".":
      letter_piece++;
      if (letter_piece >= letters[current_letter].p.length) letter_piece = 0;
      break;
  }
...
function InitializeAnimation() {
  ...
  let AnimateCallback = () => {
    ...
    if (animate_frame > TOTAL_FRAMES) {
      ...
      if (grid) DrawGrid(null, null, true, 6);
	...
	*/
      ...
      if (saving) {
        ...
        // console.log(url);
        GetFrameServerUrl(
          "/frame/" + animate_frame + "/" + encodeURIComponent(url),
          AnimateCallback
        );
      } else {
